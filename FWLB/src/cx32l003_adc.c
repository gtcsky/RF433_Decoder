...
//	assert_param(IS_ADC_TRIG1_EDGE(ADC_InitStruct->ADC_Trigs1Sel)); 
  ...
  tmpreg1 &=  (~ADC_CR0_MASK);						//重置ADC分频和通道设置
  
  /* Configure ADCx: scan conversion mode and resolution */
  /* Set SCAN bit according to ADC_ScanConvMode value */
  /* Set RES bit according to ADC_Resolution value */ 
  tmpreg1 |= (uint32_t)(((uint32_t)ADC_InitStruct->ADC_ClkSel ) | 					//设置ADC时钟分频
                        ((uint32_t)ADC_InitStruct->ADC_ChannelSel) |						//设置ADC channel
			((uint32_t)ADC_InitStruct->ADC_SamSel));							//ADC 采样周期选择
...
void ADC_CommonInit(ADC_TypeDef* ADCx,ADC_CommonInitTypeDef* ADC_CommonInitStruct)
 {
	...
	tmpreg1 |= (uint32_t) (ADC_CommonInitStruct->ADC_CtMode				//ADC_CR1.CT 设置为连续转换    单次转换/连续转换
			| ADC_CommonInitStruct->ADC_Trigs0Sel							//TRIGS0[4:0]	自动触发ADC转换设置0
			| ADC_CommonInitStruct->ADC_Trigs1Sel);							//TRIGS0[4:0]	自动触发ADC转换设置1
	/* Write to ADC CR1 */
	ADCx->CR1 = tmpreg1;
	...
	/* Set DMA bits according to ADC_DMAAccessMode value */
	/* Set DELAY bits according to ADC_TwoSamplingDelay value */
	tmpreg1 |= (uint32_t) ((ADC_CommonInitStruct->ADC_CircleMode)			//ADC 循环模式设置    0/1:非循环/循环模式
					     | (ADC_CommonInitStruct->ADC_SetAdcCnt));			//连续转换次数设置
...
void ADC_ContinuousConverChannelCmd(ADC_TypeDef* ADCx,uint32_t Channel_Enable,FunctionalState NewState)
{
  ...
  else
  {
    ...
    ADCx->CR2 &= (uint32_t)(~Channel_Enable);					//update by sky @bough.com
  }
}

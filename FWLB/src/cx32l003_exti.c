...
void GPIO_EXTILineConfig(GPIO_TypeDef* GPIOx,GPI0_IRQ_InitTypeDef* GPIO_IRQ_InitStructure, uint8_t GPIO_Pin)
{
	...
	if(GPIO_IRQ_InitStructure->GPI0_IRQ_Pin_Type) 	//GPIOx_INTTYPCR 设置中断触发类型    0/1 :边沿触发/电平触发
		GPIOx->ITYP |= GPIO_Pin;
	else
		GPIOx->ITYP  &= ~GPIO_Pin;
	
	/*!<Specifies the operating output type for the selected pins*/
	if(GPIO_IRQ_InitStructure->GPI0_IRQ_Pin_Polarity) 	//GPIOx_INTPOLCR  0:1 下降或低电平触发/上升或高电平触发
		GPIOx->IVAL |= GPIO_Pin;
	else
		GPIOx->IVAL  &= ~GPIO_Pin;
	
	/*!<Specifies the operating Pull-up/Pull down for the selected pins*/
	if(GPIO_IRQ_InitStructure->GPI0_IRQ_Pin_Edge) 	//GPIOx_INTANY 设置边沿触发类型,   0/1 :由PxIVAL决定/双沿触发
		GPIOx->IANY |= GPIO_Pin;
	else
		GPIOx->IANY  &= ~GPIO_Pin;
	
	/*!<Specifies the operating mode for the selected pins*/
	if(GPIO_IRQ_InitStructure->GPI0_IRQ_Pin_Clear)  	//GPIOx_INTCLR  0/1:保留/清除中断标志
		GPIOx->ICLR |= GPIO_Pin;
	else
		GPIOx->ICLR &= ~GPIO_Pin;
	
	/*!<Specifies the GPIO pins to be configured*/
	if(GPIO_IRQ_InitStructure->GPI0_IRQ_Pin_Enable)  	//GPIOx_INTEN	0/1 :disable/enable 外部中断
		GPIOx->IEN  |= GPIO_Pin;
	else
		GPIOx->IEN &= ~GPIO_Pin;
} 

...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef * GPIO_InitStruct)
{
	...
	for (pinpos = 0x00; pinpos < 0x08; pinpos++)
	{
		...
		if (currentpin == pos)
		{
			...
			/* SLEWCR resistor configuration*/											//IO 变化速度调节  0/1  :高速/低速
			GPIOx->SR &= ~(GPIO_PIN_OUT_MODE << (uint8_t)pinpos);
			GPIOx->SR |= (((uint32_t)GPIO_InitStruct->GPIO_Speed) << (uint8_t)pinpos);
	}
 }
}
...
{
  uint32_t temp = 0x00,tmp = 0x00,vtUser=0x00;
  ...
  vtUser= GPIOx->AFR;
  vtUser &= ~((uint32_t)0xF << ((uint32_t)((uint32_t)GPIO_PinSource & (uint32_t)0x07) * 4)) ;
  tmp = vtUser| temp;
  GPIOx->AFR  = tmp;
}

...
void ClkTrim_ClkConfig(CLKTRIM_TypeDef*CLKTRIMx,uint32_t Refclk,uint32_t Calclk)
{
  uint32_t tmpreg = 0;
	...
	/*Set Reference clock*/
	tmpreg  &= ~REFCLK_MASK;
	tmpreg  |= Refclk;						//REFCLK_SEL[2:0] 		ѡ��ο�ʱ��
	
	/*Set Calibration clock*/
	tmpreg  &= ~CALCLK_MASK;
	tmpreg  |= Calclk;						//CALCLK_SEL[1:0]		ѡ���У׼ʱ��
...
void ClkTrim_StartCmd(CLKTRIM_TypeDef*CLKTRIMx,FunctionalState NewState)
{
   ...
   if(NewState != DISABLE)
	 ...
	 else
	 {
		  ...
		  CLKTRIMx->CR &= ~CLK_TMRIM_START;					//TRIM_START   ��ʼУ׼
	 }
}
   ...
   if(NewState != DISABLE)
	 {
		  ...
		  CLKTRIMx->CR |= CLK_MON_ENABLE;			//����ģʽʹ�ܼĴ�����
	 }
	 else
	 {
...
void ClkTrim_ClkCmd(CLKTRIM_TypeDef*CLKTRIMx,FunctionalState NewState)
{
   ...
   if(NewState != DISABLE)
	 ...
	 else
	 {
		  ...
		  CLKTRIMx->CR &= ~CLK_REFCALCLK_ENABLE;				//CLKTRIM_CR.CLKEN=1     enable�ο�ʱ�Ӻ�У��ʱ��
	 }
}
...
void ClkTrim_SetRcntValue(CLKTRIM_TypeDef*CLKTRIMx,uint32_t Value)
{
	...
	/*Set value to REFCON register*/
	 CLKTRIMx->REFCON = Value;							//CLKTRIM_REFCON  ���òο���������ֵ
}
...
void ClkTrim_SetCalValue(CLKTRIM_TypeDef*CLKTRIMx,uint32_t Value)
{
   ...
   CLKTRIMx->CALCON = Value;											//CLKTRIM_CALCON	����У׼���������ֵ
}

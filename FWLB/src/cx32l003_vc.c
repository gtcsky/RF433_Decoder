...
void VC_SetV25Div(VC_TypeDef *VCx,uint32_t V25DIV)
{
  ...
  assert_param(VC_V25DIV_SEL(V25DIV));
	...
	/*Set Nin*/
	VCx->PCR |= V25DIV;							//V25DIV[1:0] 00: 1/4  V25		01:2/4 V25    10:3/4 V25   11:V25
}
...
void VC_V25DivCmd(VC_TypeDef *VCx,FunctionalState NewState)
{
  ...
  assert_param(IS_FUNCTIONAL_STATE(NewState));
	...
	if(NewState != DISABLE)
		...
		/*close v25div */
		VCx->PCR &= ~V25DIV_ENABLE;   					//V25DIV _EN     V25分频控制开关
}
...
void VC_SetFltClk(VC_TypeDef *VCx,uint32_t FLTCLK)
 {
	...
	VCx->CR |= FLTCLK;						//VC_FLTCLK_SEL[1:0] 数字滤波时钟选择  01:PCLK  10:clk_38.4K(26us/周期)
}
...
void VC_FLTCmd(VC_TypeDef *VCx,FunctionalState NewState)
 {
	...
	if (NewState != DISABLE)
		/*enable flt function */
		VCx->CR |= VC_FLT_ENABLE;						//FLTEN  数字滤波开关
	else
...
void VC_HighITCmd(VC_TypeDef *VCx,FunctionalState NewState)
 {
	...
	if (NewState != DISABLE)
		...
		VCx->CR |= VC_HIGHINT_ENABLE;					//HIGHINTEN		高电平触发使能 (被监测电压低于阈值电压)
	else
...
void VC_ITConfig(VC_TypeDef *VCx,FunctionalState NewState)
 {
	...
	if (NewState != DISABLE)
		...
		VCx->CR |= VC_TI_ENABLE;					//INT_EN  VC中断开关
	else

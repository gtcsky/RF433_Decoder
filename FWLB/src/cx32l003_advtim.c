...
/** @defgroup ADVTIM1 
  * @brief ADVTIM1 driver modules
  * @{
  */

/** @defgroup ADVTIM_Private_TypesDefinitions
  * @{
  */
...
/** @defgroup ADVTIM_Private_Defines
  * @{
  */
...
/** @defgroup ADVTIM_Private_Macros
  * @{
  */
...
/** @defgroup ADVTIM_Private_Variables
  * @{
  */
...
/** @defgroup ADVTIM_Private_FunctionPrototypes
  * @{
  */

static void TI1_Config(ADVTIM_TypeDef* TIMx, uint16_t TIM_ICPolarity, uint16_t TIM_ICSelection,
                       uint16_t TIM_ICFilter);
static void TI2_Config(ADVTIM_TypeDef* TIMx, uint16_t TIM_ICPolarity, uint16_t TIM_ICSelection,
                       uint16_t TIM_ICFilter);
static void TI3_Config(ADVTIM_TypeDef* TIMx, uint16_t TIM_ICPolarity, uint16_t TIM_ICSelection,
                       uint16_t TIM_ICFilter);
static void TI4_Config(ADVTIM_TypeDef* TIMx, uint16_t TIM_ICPolarity, uint16_t TIM_ICSelection,
                       uint16_t TIM_ICFilter);
/**
  * @}
  */
...
/** @defgroup ADVTIM_Private_Functions
  * @{
  */
...
void ADVTIM_TimeBaseInit(ADVTIM_TypeDef* TIMx, ADVTIM_TimeBaseInitTypeDef* TIM_TimeBaseInitStruct)
{
  ...
  /* Check the parameters */
  assert_param(IS_TIM_ALL_PERIPH(TIMx)); 
  assert_param(IS_ADTIM_COUNTER_MODE(TIM_TimeBaseInitStruct->TIM_CounterMode));
...
//  if((TIMx != TIM6) && (TIMx != TIM7))
  ...
  TIMx->CR1 = tmpcr1;												//TIM1_CR1.b4  设置计数器向上计数

  /* Set the Autoreload value */
  TIMx->ARR = TIM_TimeBaseInitStruct->TIM_Period ;					//TIM1_ARR  自动重载寄存器,在PWM模式用于确定PWM的频率.    用TIM1_CCRx确定占空比
 
  /* Set the Prescaler value */
  TIMx->PSC = TIM_TimeBaseInitStruct->TIM_Prescaler;					//预分频器设置
			
//  if ((TIMx == TIM1) || (TIMx == TIM8)|| (TIMx == TIM15)|| (TIMx == TIM16) || (TIMx == TIM17))  
  if((TIMx == ADVTIM1) || (TIMx == ADVTIM2))
  {	
    /* Set the Repetition Counter value */
    TIMx->RCR = TIM_TimeBaseInitStruct->TIM_RepetitionCounter;			//TIM1_RCR	REP[7:0] /这意味着在 PWM 模式中，(REP+1)对应着： － 在边沿对齐模式下，PWM 周期的数目；
  }	
...
/**
  ...
  * @param  TIM_OCInitStruct: pointer to a ADVTIM_OCInitTypeDef structure
...
void ADVTIM_OC1Init(ADVTIM_TypeDef* TIMx, ADVTIM_OCInitTypeDef* TIM_OCInitStruct)
{
 ...
 /* Disable the Channel 1: Reset the CC1E Bit */
  TIMx->CCER &= (uint16_t)(~(uint16_t)TIM_CCER_CC1E);			//TIM1_CCER   0：捕获禁止。 该位决定了计数器的值是否能捕获入 TIM1_CCR1 寄存器。
  /* Get the TIMx CCER register value */
  tmpccer = TIMx->CCER;									//TIM1_CCER
  /* Get the TIMx CR2 register value */
  tmpcr2 =  TIMx->CR2;										//TIM1_CR2
  
  /* Get the TIMx CCMR1 register value */
  tmpccmrx = TIMx->CCMR1;								//TIM1_CCMR1
    
  /* Reset the Output Compare Mode Bits */
  tmpccmrx &= (uint16_t)(~((uint16_t)TIM_CCMR1_OC1M));		//重置OC1M[2:0] =000：冻结。输出比较寄存器 TIM1_CCR1 与计数器 TIM1_CNT 间的比较对 OC1REF 不起作用；

  tmpccmrx &= (uint16_t)(~((uint16_t)TIM_CCMR1_CC1S));		//重置CC1S[1:0] =00：CC1 通道被配置为输出；

  /* Select the Output Compare Mode */
  tmpccmrx |= TIM_OCInitStruct->TIM_OCMode;				//设置OC1M[2:0]
  
  /* Reset the Output Polarity level */
  tmpccer &= (uint16_t)(~((uint16_t)TIM_CCER_CC1P));			//重置 CC1P
  /* Set the Output Compare Polarity */
  tmpccer |= TIM_OCInitStruct->TIM_OCPolarity;					//设置CC1P  CC1 通道配置为输出时 	0/1:	OC1 高/低电平有效；
  
  /* Set the Output State */
  tmpccer |= TIM_OCInitStruct->TIM_OutputState;				//设置CC1E  CC1 通道配置为输出时	0/1：关闭/开启输出到对应的引脚
...
//     (TIMx == TIM16)|| (TIMx == TIM17))
  if((TIMx == ADVTIM1) || (TIMx == ADVTIM2))		
  {
    ...
    tmpccer |= TIM_OCInitStruct->TIM_OCNPolarity;			//设置CC1NP 输入/捕获 1 互补输出极性  0/1:	OC1 高/低电平有效；
    
    /* Reset the Output N State */
    tmpccer &= (uint16_t)(~((uint16_t)TIM_CCER_CC1NE));    
    /* Set the Output N State */
    tmpccer |= TIM_OCInitStruct->TIM_OutputNState;			//设置CC1NE 输入/捕获 1 互补输出使能   0/1：关闭/开启输出到对应的引脚
    ...
    tmpcr2 |= TIM_OCInitStruct->TIM_OCIdleState;			//设置OIS1  输出空闲状态 1(OC1 输出)    0/1:	OC1=0/1;
    /* Set the Output N Idle state */
    tmpcr2 |= TIM_OCInitStruct->TIM_OCNIdleState;			//设置OIS1N  输出空闲状态 1(OC1N 输出)    0/1:	OC1N=0/1;
  }
  /* Write to TIMx CR2 */
  TIMx->CR2 = tmpcr2;
  ...
  TIMx->CCR1 = TIM_OCInitStruct->TIM_Pulse; 				//CCR1[15:0] 包含了装入当前捕获/比较 1 寄存器的值(预装载值) 当前捕获/比较寄存器参与同计数器 TIM1_CNT 的比较，并在OC1 端口上产生输出信号
...
/**
  ...
  * @param  TIMx: where x can be  1 to 17 except 6 and 7 to select the TIM peripheral.
  * @param  TIM_ICInitStruct: pointer to a ADVTIM_ICInitTypeDef structure
  *         that contains the configuration information for the specified TIM peripheral.
  * @retval None
  */
...
//     (TIMx == TIM4) ||(TIMx == TIM5))
  ...
  if (TIM_ICInitStruct->TIM_Channel == TIM_Channel_1)
  {
    ...
    ADVTIM_SetIC1Prescaler(TIMx, TIM_ICInitStruct->TIM_ICPrescaler);
  }
  else if (TIM_ICInitStruct->TIM_Channel == TIM_Channel_2)
  {
    ...
    TI2_Config(TIMx, TIM_ICInitStruct->TIM_ICPolarity,
               ...
               TIM_ICInitStruct->TIM_ICFilter);
    /* Set the Input Capture Prescaler value */
    ADVTIM_SetIC2Prescaler(TIMx, TIM_ICInitStruct->TIM_ICPrescaler);
  }
  else if (TIM_ICInitStruct->TIM_Channel == TIM_Channel_3)
  {
    ...
    TI3_Config(TIMx,  TIM_ICInitStruct->TIM_ICPolarity,
               ...
               TIM_ICInitStruct->TIM_ICFilter);
    /* Set the Input Capture Prescaler value */
    ADVTIM_SetIC3Prescaler(TIMx, TIM_ICInitStruct->TIM_ICPrescaler);
  }
  else
  {
    assert_param(IS_TIM_LIST3_PERIPH(TIMx));
    ...
    TI4_Config(TIMx, TIM_ICInitStruct->TIM_ICPolarity,
               ...
               TIM_ICInitStruct->TIM_ICFilter);
    /* Set the Input Capture Prescaler value */
    ADVTIM_SetIC4Prescaler(TIMx, TIM_ICInitStruct->TIM_ICPrescaler);
  } 
}
...
/**
  ...
  * @param  TIM_BDTRInitStruct: pointer to a ADVTIM_BDTRInitTypeDef structure that
...
/**
  ...
  * @param  TIM_OCInitStruct : pointer to a ADVTIM_OCInitTypeDef structure which will
  *         be initialized.
  * @retval None
  */
...
/**
  ...
  * @param  TIM_ICInitStruct: pointer to a ADVTIM_ICInitTypeDef structure which will
  *         be initialized.
  * @retval None
  */
...
/**
  ...
  * @param  TIM_BDTRInitStruct: pointer to a ADVTIM_BDTRInitTypeDef structure which
  *         will be initialized.
  * @retval None
  */
...
void ADVTIM_CtrlPWMOutputs(ADVTIM_TypeDef* TIMx, FunctionalState NewState)
{
  ...
  else
  {
    ...
    TIMx->BDTR &= (uint16_t)(~((uint16_t)TIM_BDTR_MOE));		//主输出使能 	1：如果设置了相应的使能位(TIM1_CCER 寄存器的 CCxE、CCxNE 位)，则开启 OC 和 OCN 输出。
  }  
}
...
/**
  ...
  * @retval None
  */
void ADVTIM_DMAConfig(ADVTIM_TypeDef* TIMx, uint16_t TIM_DMABase, uint16_t TIM_DMABurstLength)
{
  /* Check the parameters */
...
/**
  ...
  * @retval None
  */
void ADVTIM_DMACmd(ADVTIM_TypeDef* TIMx, uint16_t TIM_DMASource, FunctionalState NewState)
{ 
  /* Check the parameters */
...
void ADVTIM_ITRxExternalClockConfig(ADVTIM_TypeDef* TIMx, uint16_t TIM_InputTriggerSource)
{
  ...
  ADVTIM_SelectInputTrigger(TIMx, TIM_InputTriggerSource);
                                ...
                                uint16_t TIM_ICPolarity, uint16_t ICFilter)
{
  ...
  ADVTIM_SelectInputTrigger(TIMx, TIM_TIxExternalCLKSource);
  /* Select the External clock mode1 */
  TIMx->SMCR |= TIM_SlaveMode_External1;
}
                             ...
                             uint16_t ExtTRGFilter)
{
  ...
  ADVTIM_ETRConfig(TIMx, TIM_ExtTRGPrescaler, TIM_ExtTRGPolarity, ExtTRGFilter);
...
void ADVTIM_CounterModeConfig(ADVTIM_TypeDef* TIMx, uint16_t TIM_CounterMode)
{
  ...
  /* Check the parameters */
  assert_param(IS_TIM_LIST3_PERIPH(TIMx));
  assert_param(IS_ADTIM_COUNTER_MODE(TIM_CounterMode));
  tmpcr1 = TIMx->CR1;
